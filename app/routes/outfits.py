...
def patch_outfit(outfit_id: str):
    ...
    outfit = outfit_manager.patch_outfit(
        g.user_id,
        outfit_id,
        name=name,
...
def generate_outfits():
    ...
    outfits = outfit_manager.generate_outfit(
        g.user_id, seasons=typed_seasons, tags=typed_tags, anchor=anchor, amount=amount
    )

    return jsonify(
        {"outfits": [outfit.to_dict() for outfit in outfits], "count": len(outfits)}
    ), 200
